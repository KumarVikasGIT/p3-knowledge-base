...
category: galleries
excerpt: 'From the Share panel on a gallery you own, you have two related but different actions: Regenerate
  link and Disable sharing. They''re not interchangeable, so it''s worth understanding what each one...'
author: Vikas Kumbhakar
migration_original_priority: high
migration_note: Must distinguish "regenerate" from "disable sharing" precisely — they have different effects
  per canonical spec (isPublic=false is a hard server-side revoke; regenerate only invalidates the old
  token going forward).
---
